...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        ...
        <!-- simple xml file as a start containing all the paths of the multi-file data -->
        <xsl:for-each select="//file">
            ...
            <!-- loop over all pages -->
            <xsl:for-each-group select="document($fileName)/TEI/text/body//lg/l" group-starting-with="/TEI/text/body//lg/l[pb]">
                ...
                <xsl:result-document href="{$outputFile}" method="text" encoding="UTF-8" indent="yes">
                    ...
                    <!-- for eachof these page groups print the lines -->
                    <xsl:for-each select="current-group()"><!-- XSL 2.0 only -->
                        <xsl:if test="position()!=1"> ,</xsl:if>    
                        {
                        ...
                        "chars":"<xsl:value-of select="normalize-space($lineText)"></xsl:value-of>", <!-- exclude of notes, emph, etc... -->
                        "format":"text/plain",
                        ...
                        <xsl:variable name="facsID"><xsl:value-of select="substring-after(@facs,'#')"/></xsl:variable>
                        <!-- upper left x  -->
                        <xsl:variable name="x"><xsl:value-of select="/TEI/facsimile/surface/zone[@xml:id=$facsID]/@ulx"/></xsl:variable>
                        <!-- upper left y  -->
                        <xsl:variable name="y"><xsl:value-of select="/TEI/facsimile/surface/zone[@xml:id=$facsID]/@uly"/></xsl:variable> 
                        <!-- width = lower right x  -  upper left x  -->
                        <xsl:variable name="w"><xsl:value-of select="/TEI/facsimile/surface/zone[@xml:id=$facsID]/@lrx - /TEI/facsimile/surface/zone[@xml:id=$facsID]/@ulx"/></xsl:variable>
                        <!-- height = lower right y  -  upper left y  -->
                        <xsl:variable name="h"><xsl:value-of select="/TEI/facsimile/surface/zone[@xml:id=$facsID]/@lry - /TEI/facsimile/surface/zone[@xml:id=$facsID]/@uly"/></xsl:variable> 
